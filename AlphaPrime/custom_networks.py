...
# Define string version of real_dtype
if real_dtype == tf.float32:
    real_dtype_str = 'float32'
else:
    real_dtype_str = 'float64'
...
class bihom_function_generator_old(tf.Module):
    ...
    def __call__(self,points):
        ...
        #cpoints_stored=tf.TensorArray(complex_dtype,size=nProjective)
...
class bihom_function_generatorQorT(tf.Module):
    ...
    def __call__(self, points):
        ...
        points0=tf.einsum('xi,x->xi',points,tf.cast(tf.reduce_sum(tf.abs(points)**2, axis=-1)**(-0.5),complex_dtype))
...
#     matrixNby1ofOnes=tf.cast(tf.ones_like(cpoints[:,0:1]),complex_dtype)
...
class get_degree_kphiandMmonomials_general(tf.Module):
    ...
    def __call__(self,cpoints):
        ...
        for i in range(0,len(self.linebundleindices)):
            ...
            s = self.slice_indices[i,0]
            e= self.slice_indices[i,1]
            kappa_i=tf.cast(tf.reduce_sum(tf.math.abs(cpoints[:,s:e])**2,axis=-1),complex_dtype)
            if self.linebundleindices[i]>=0:
                ...
                #print(monsbar)
                #print(mons)
                kappa_i_kphiplusmodM=kappa_i**tf.cast((self.kphi[i]+tf.math.abs(self.linebundleindices[i])),complex_dtype)
...
class SquareDenseVar(tf.keras.layers.Layer):
    def __init__(self, input_dim, units, activation=tf.square, stddev=0.05,trainable=True,positive_init=True):
        ...
        self.w = tf.Variable(
            initial_value=tf.math.abs(w_init(shape=(input_dim, units), dtype=real_dtype_str)) if positive_init else w_init(shape=(input_dim, units), dtype=real_dtype_str),
            trainable=trainable,
        )
...
class SquareDenseVarNoAct(tf.keras.layers.Layer):
    def __init__(self, input_dim, units, stddev=0.05,trainable=True,positive_init=True):
        ...
        w_init = tf.random_normal_initializer(mean=0.0, stddev=stddev)
        self.w = tf.Variable(
            #initial_value=tf.math.abs(w_init(shape=(input_dim, units), dtype=real_dtype_str)),
            initial_value=tf.math.abs(w_init(shape=(input_dim, units), dtype=real_dtype_str)) if positive_init else w_init(shape=(input_dim, units), dtype=real_dtype_str),
            #initial_value=w_init(shape=(input_dim, units), dtype=real_dtype_str),
            trainable=trainable,
        )
    @tf.function
    def call(self, inputs):
...
class BiholoModelFuncGENERAL_BROKEN(tf.keras.Model):
    def __init__(self, layer_sizes,BASIS,activation=tf.square,stddev=0.1,use_zero_network=False):
        ...
        self.layers_list = [tfk.layers.Dense(units=layer_sizes[i+1],activation=activation, input_shape=(layer_sizes[i],) if i == 0 else None)
                            for i in range(len(layer_sizes)-2-1)]#i.e. 0->1,1->2,... layer_sizes-2->layer_sizes-3->layer_sizes-2. so misses the last 1. this should be 1.
...
class BiholoModelFuncGENERAL2(tf.keras.Model):
    def __init__(self, layer_sizes, BASIS, activation=tf.square, stddev=0.1, use_zero_network=False):
        super().__init__()
        set_stddev = stddev

        self.layers_list = []
        self.layers_list.append(tf.keras.layers.Input(shape=(25,)))
        for i in range(1):#len(layer_sizes) - 2):
            layer = tfk.layers.Dense(
                units=layer_sizes[i+1],
                activation=activation,
                #input_shape=(layer_sizes[i],) if i == 0 else None
            )
            self.layers_list.append(layer)

        # Add the final layer without activation
        final_layer = tfk.layers.Dense(units=layer_sizes[-1], activation=None)
        self.layers_list.append(final_layer)

        # Add the extra free parameter layer
        extra_layer = tf.keras.layers.Dense(
            units=1,
            #use_bias=False,
            #kernel_initializer='zeros' if use_zero_network else 'glorot_uniform'
        )
        self.layers_list.append(extra_layer)
        self.BASIS=BASIS
        self.nCoords=tf.reduce_sum(tf.cast(BASIS['AMBIENT'],tf.int32)+1)
        self.ambient=tf.cast(BASIS['AMBIENT'],tf.int32)
        self.kmoduli=BASIS['KMODULI']
