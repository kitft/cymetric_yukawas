...
def train_model(fsmodel, data, optimizer=None, epochs=50, batch_sizes=[64, 10000],
    ...
    # Prepare datasets outside the loop
    dataset1 = tf.data.Dataset.from_tensor_slices(
        (tf.cast(data['X_train'], real_dtype), tf.cast(data['y_train'], real_dtype), tf.cast(data['train_pullbacks'], complex_dtype))).cache()
    
    # Create both batched datasets upfront
    batch_size1 = batch_sizes[0]
    batch_size2 = min(batch_sizes[1], len(data['X_train']))
    
    steps_per_epoch1 = max(1, len(data['X_train']) // batch_size1)
    steps_per_epoch2 = max(1, len(data['X_train']) // batch_size2)
    
    batched_dataset1 = dataset1.batch(batch_size1).prefetch(tf.data.AUTOTUNE).repeat()
    batched_dataset2 = dataset1.batch(batch_size2).prefetch(tf.data.AUTOTUNE).repeat()
