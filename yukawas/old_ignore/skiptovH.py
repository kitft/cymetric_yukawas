...
nPoints=300000# is the same as below
nPointsHF=300000
tr_batchsize=64
...
nEpochsBeta=2#40
nEpochsBeta=2
nEpochsSigma=100#changed below vH

depthPhi=3
widthPhi=3#64#128 4 in the 1.0s
depthBeta=3
widthBeta=3#64
depthSigma=3
widthSigma=3#64 # up from 256
alphabeta=[1,10]
...
if __name__ ==  '__main__':
    ...
    if train_phi:
        ...
        phimodel1,training_history=train_and_save_nn(free_coefficient,depthPhi,widthPhi,nEpochsPhi,stddev=0.05,bSizes=[64,50000],lRate=lRatePhi) 
    else:
    ...
    generate_points_and_save_using_defaultsHYM(free_coefficient,linebundleforHYM_02m20,nPoints,phimodel1,force_generate=False,seed_set=seed_for_gen)
    if train_02m20:
    ...
    #
    #    generate_points_and_save_using_defaultsHYM(free_coefficient,linebundleforHYM_110m2,nPoints,phimodel1,force_generate=False,seed_set=seed_for_gen)
    #    if train_110m2:
    ...
    #    delete_all_dicts_except('dataEval')
    #
    #    generate_points_and_save_using_defaultsHYM(free_coefficient,linebundleforHYM_m1m322,nPoints,phimodel1,force_generate=False,seed_set=seed_for_gen)
    #    if train_m1m322:
    ...
    gc.collect()
    tf.keras.backend.clear_session()
    mem2 = tracker.SummaryTracker()
    print(sorted(mem2.create_summary(), reverse=True, key=itemgetter(2))[:10])
    print(mem.print_diff())
    print("perm")
    print(perm.print_diff())
    delete_all_dicts_except('dataEval')
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_02m20,functionforbaseharmonicform_jbar_for_vH,phi,betamodel_02m20,nPointsHF,force_generate=True,seed_set=seed_for_gen)
    if train_vH:
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_110m2,functionforbaseharmonicform_jbar_for_vQ3,phi,betamodel_110m2,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    if train_vQ3:
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_110m2,functionforbaseharmonicform_jbar_for_vU3,phi,betamodel_110m2,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_m1m322,vFS_Q1,phi,betamodel_m1m322,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    if train_vQ1:
        HFmodel_vQ1,trainingHistoryHF_vQ1=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q1,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma2)
    else:
        HFmodel_vQ1,trainingHistoryHF_vQ1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q1,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma2)
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_m1m322,vFS_Q2,phi,betamodel_m1m322,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    if train_vQ2:   
        HFmodel_vQ2,trainingHistoryHF_vQ2=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q2,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma2)
    else:
        HFmodel_vQ2,trainingHistoryHF_vQ2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q2,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma2)
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_m1m322,vFS_U1,phi,betamodel_m1m322,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    if train_vU1:
        HFmodel_vU1,trainingHistoryHF_vU1=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U1,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma2)
    else:
        HFmodel_vU1,trainingHistoryHF_vU1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U1,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma2)
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    generate_points_and_save_using_defaultsHF(free_coefficient,linebundleforHYM_m1m322,vFS_U2,phi,betamodel_m1m322,nPointsHF,force_generate=False,seed_set=seed_for_gen)
    if train_vU2:
        HFmodel_vU2,trainingHistoryHF_vU2=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U2,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma2)
    else:
        HFmodel_vU2,trainingHistoryHF_vU2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U2,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma2)
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    import csv
    filename = 'holo_training_results.csv'
    with open(filename, mode='a') as file:
        writer = csv.writer(file)
        writer.writerow([free_coefficient,nEpochsPhi,nEpochsBeta,nEpochsSigma,widthPhi,widthBeta,widthSigma,nPoints,nPointsHF,depthPhi,depthBeta,depthSigma,mats,masses_ref_and_learned])
        file.close()
