...
def laplacian_measure_loss(model, validation_data, batch=False):
    ...
    Args:
        ...
        batch (bool): Whether to use batching
    ...
    # Use TensorArray for proper scoping in graph mode
    batch_size = 1000
    num_samples = tf.shape(X_val)[0]
    num_batches = tf.cast(tf.math.ceil(num_samples / batch_size), tf.int32)
    
    # Initialize TensorArray to store results
    results_array = tf.TensorArray(dtype=real_dtype, size=num_batches, dynamic_size=False)
    ...
    def body(i, results_array):
        ...
        results_array = results_array.write(i, batch_result)
        
        return i + 1, results_array
    
    # Run the while loop
    _, results_array = tf.while_loop(condition, body, [i, results_array])
    
    # Stack all results and compute mean
    all_results = results_array.concat()
    return tf.reduce_mean(all_results)

# Apply tf.function to the loss function
