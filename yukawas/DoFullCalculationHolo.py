...
#nEpochsSigma2=3#changed below vH
...
#depthPhi=2
#widthPhi=8#12
#depthBeta=2   #DepthBeta=2
#widthBeta=8  #WidthBeta=8
#depthSigma=2  #DepthSigma=2
#widthSigma=8 #WidthSigma=128 # up from 256
#depthSigma2=2 #DepthSigma2=2
#widthSigma2=8#WidthSigma2=196 # up from 256
...
#lRateSigma2=0.005# perhaps best as 0.03
...
#stddev_H33=0.5
#final_layer_scale_H33=0.01
stddev_H33=0.5
final_layer_scale_H=1
final_layer_scale_33=0.1

force_generate_phi=True
force_generate_HYM=True
force_generate_HF=False

skip_measures_phi = True
skip_measures_HYM = True
skip_measures_HF = False
...
if __name__ ==  '__main__':
    ...
    if train_phi:
        #phimodel1,training_history,measure_phi=train_and_save_nn(free_coefficient,depthPhi,widthPhi,nEpochsPhi,bSizes=[64,tr_batchsize],lRate=lRatePhi) 
        phimodel1,training_history,measure_phi=train_and_save_nn(free_coefficient,depthPhi,widthPhi,nEpochsPhi,stddev=0.05,bSizes=[64,SecondBSize],lRate=lRatePhi) 
    else:
        phimodel1,training_history,measure_phi=load_nn_phimodel(free_coefficient,depthPhi,widthPhi,nEpochsPhi,[64,SecondBSize],set_weights_to_zero=return_zero_phi,skip_measures=skip_measures_phi)
    ...
    if train_02m20:
        betamodel_02m20,training_historyBeta_d02m20,measure_LB1=train_and_save_nn_HYM(free_coefficient,linebundleforHYM_02m20,depthBeta,widthBeta,nEpochsBeta,stddev=0.05,bSizes=[64],lRate=lRateBeta,alpha=alphabeta,load_network=False)
    else: 
        betamodel_02m20,training_historyBeta_02m20,measure_LB1=load_nn_HYM(free_coefficient,linebundleforHYM_02m20,depthBeta,widthBeta,nEpochsBeta,[64],set_weights_to_zero=return_zero_HYM,skip_measures=skip_measures_HYM)
    ...
    #mbetamodel_02m20,mtraining_historyBeta_02m20,measure_?=train_and_save_nn_HYM(free_coefficient,-linebundleforHYM_02m20,depthBeta,widthBeta,nEpochsBeta,[64],0.001,alpha=alphabeta)
    ...
    if train_110m2:
        ...
        betamodel_110m2,training_historyBeta_110m2,measure_LB2=load_nn_HYM(free_coefficient,linebundleforHYM_110m2,depthBeta,widthBeta,nEpochsBeta,[64],set_weights_to_zero=return_zero_HYM,skip_measures=skip_measures_HYM)
    #mem= = tracker.SummaryTracker()
    #print(sorted(mem(.create_summary(), reverse=True, key=itemgetter(2))[:10])
    delete_all_dicts_except('dataEval')
    gc.collect()
    ...
    if train_m1m322:
        ...
        betamodel_m1m322,training_historyBeta_m1m322,measure_LB3=load_nn_HYM(free_coefficient,linebundleforHYM_m1m322,depthBeta,widthBeta,nEpochsBeta,[64],set_weights_to_zero=return_zero_HYM,skip_measures=skip_measures_HYM)
    ...
    if train_vH:
        HFmodel_vH,trainingHistoryHF_vH,measure_vH=train_and_save_nn_HF(free_coefficient,linebundleforHYM_02m20,betamodel_02m20,functionforbaseharmonicform_jbar_for_vH,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_H)
    else:
        HFmodel_vH,trainingHistoryHF_vH,measure_vH=load_nn_HF(free_coefficient,linebundleforHYM_02m20,betamodel_02m20,functionforbaseharmonicform_jbar_for_vH,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma1,skip_measures=skip_measures_HF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    if train_vQ3:
        HFmodel_vQ3,trainingHistoryHF_vQ3,measure_vQ3=train_and_save_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vQ3,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_33)
    else:
        HFmodel_vQ3,trainingHistoryHF_vQ3,measure_vQ3=load_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vQ3,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma1,skip_measures=skip_measures_HF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    #print(sorted(mem(.create_summary(), reverse=True, key=itemgetter(2))[:10])
    if train_vU3:
        HFmodel_vU3,trainingHistoryHF_vU3,measure_vU3=train_and_save_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vU3,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_33)
    else:
        HFmodel_vU3,trainingHistoryHF_vU3,measure_vU3=load_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vU3,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma1,skip_measures=skip_measures_HF)
    ...
    if train_vQ1:
        HFmodel_vQ1,trainingHistoryHF_vQ1,measure_vQ1=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q1,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vQ1,trainingHistoryHF_vQ1,measure_vQ1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q1,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measures_HF)
    ...
    if train_vQ2:   
        HFmodel_vQ2,trainingHistoryHF_vQ2,measure_vQ2=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q2,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vQ2,trainingHistoryHF_vQ2,measure_vQ2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_Q2,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measures_HF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    if train_vU1:
        HFmodel_vU1,trainingHistoryHF_vU1,measure_vU1=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U1,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vU1,trainingHistoryHF_vU1,measure_vU1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U1,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measures_HF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    if train_vU2:
        HFmodel_vU2,trainingHistoryHF_vU2,measure_vU2=train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U2,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vU2,trainingHistoryHF_vU2,measure_vU2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,vFS_U2,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measures_HF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    pg,kmoduli=generate_points_and_save_using_defaults_for_eval(free_coefficient,n_integrate,seed_set=seed_for_gen)
    ...
    import csv
    filename = '1809holo_training_results_fixed.csv'
    ...
    with open(filename, mode='a') as file:
        ...
        writer.writerow([free_coefficient,nEpochsPhi,nEpochsBeta,nEpochsSigma,nEpochsSigma2,widthPhi,widthBeta,widthSigma, widthSigma2,nPoints,nPointsHF,depthPhi,depthBeta,depthSigma,depthSigma2,mats,masses_ref_and_learned,measures])
        file.close()
